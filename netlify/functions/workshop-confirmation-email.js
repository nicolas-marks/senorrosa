...
function buildConfirmationEmail({ name, email, receiptUrl, amountCents = WORKSHOP_EVENT.priceCents }) {
  const safeName = escapeHtml(name);
  const paidAmount = `$${(amountCents / 100).toFixed(2)}`;
  ...
  return {
    ...
    subject: "You're registered — Practical AI Workshop | Señor Rosa Digital Literacy",
    text: `Hi ${name},\n\nYour registration and ${paidAmount} payment are confirmed.\n\nPractical AI Workshop\nSeñor Rosa Digital Literacy\nSeptember 26, 2026\n1:00 PM–4:00 PM\nBroadway Commons\nSalem, Oregon\n\nWORKSHOP ONBOARDING\n${TRAINING_URL}\nBring your laptop, charger, Google account, and useful non-sensitive business materials. ChatGPT Plus is required — $20/month, purchased separately from OpenAI. Your workshop payment does not include Plus; Señor Rosa receives no part of that subscription.\n\nOFFICIAL SQUARE RECEIPT${receiptText || "\nYour receipt link was not available in the payment response."}\n\nQuestions? Reply to this email or contact info@senorrosa.com.`,
    html: `<div style="font-family:Arial,sans-serif;max-width:600px;color:#2c1822;line-height:1.6"><h1 style="font-size:24px">You're registered.</h1><p>Hi ${safeName},</p><p>Your registration and <strong>${paidAmount} payment</strong> are confirmed.</p><h2 style="font-size:18px">Practical AI Workshop</h2><p>Señor Rosa Digital Literacy</p><p>September 26, 2026<br>1:00 PM–4:00 PM<br>Broadway Commons<br>Salem, Oregon</p><h2 style="font-size:18px">Workshop onboarding</h2><p><a href="${TRAINING_URL}">${TRAINING_URL}</a></p><p>Bring your laptop, charger, Google account, and useful non-sensitive business materials. ChatGPT Plus is required — $20/month, purchased separately from OpenAI. Your workshop payment does not include Plus; Señor Rosa receives no part of that subscription.</p><h2 style="font-size:18px">Official Square receipt</h2>${receiptHtml || "<p>Your receipt link was not available in the payment response.</p>"}<p>Questions? Reply to this email or contact info@senorrosa.com.</p></div>`
  };
}
...
async function sendWorkshopConfirmation({ paymentId, name, email, receiptUrl, amountCents = WORKSHOP_EVENT.priceCents, currency = WORKSHOP_EVENT.currency }) {
  const apiKey = process.env.RESEND_API_KEY;
  if (!apiKey) return { status: "not_configured", organizerStatus: "not_configured" };

  const attendeePayload = buildConfirmationEmail({ name, email, receiptUrl, amountCents });
  const policyText = "Your workshop registration fee is refundable if you cancel more than 48 hours before the workshop. Cancellations within 48 hours are nonrefundable. If Señor Rosa cancels, you receive a full refund of the workshop fee and retain the training materials. Your separate ChatGPT Plus subscription is managed through OpenAI.";
